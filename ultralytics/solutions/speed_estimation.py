...
class SpeedEstimator:
    ...
    def plot_box_and_track(self, track_id, box, cls, track, video):
        """
        ...
        if(video == "catherine"):
            if track_id in self.dist_data:
                if (int(self.dist_data[track_id])) > 1 and (int(self.dist_data[track_id])) <= 5: # speed threshold
                    label = "Moving Car"
                else:
                    label = "Parked Car"
                    parked_car = 1
                    print(f"Parked car detected! track_id:{track_id}")
                    parked_cars_track_ids.append(track_id)
        else: # mcgill video
            if track_id in self.dist_data:
                if (int(self.dist_data[track_id])) >= 2 and (int(self.dist_data[track_id])) <= 20: # speed threshold
                    label = "Parked Car"
                    print(f"Parked car detected! track_id:{track_id}")
                    parked_cars_track_ids.append(track_id)
                else:
        ...
        # END
        speed_label = f"{label}" if track_id in self.dist_data else label
    ...
    def estimate_speed(self, im0, tracks, region_color=(255, 0, 0), video="catherine"):
        """
        ...
        for box, trk_id, cls in zip(self.boxes, self.trk_ids, self.clss):
            ...
            parked_cars_track_ids = self.plot_box_and_track(trk_id, box, cls, track, video)
            self.calculate_speed(trk_id, track)
        if self.view_img and self.env_check:
            self.display_frames()

        return im0, parked_cars_track_ids
